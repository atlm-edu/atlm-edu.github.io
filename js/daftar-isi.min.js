...
(function(win, doc) {
    ...
    win['clickTabs_' + hash] = function(pos) {
        ...
        for (var u = 0, uen = d.length; u < uen; ++u) {
            d[u].className = "";
            d[parseInt(pos, 10)].className = "active";
        }
    };
    ...
    win['showTabs_' + hash] = function(json) {
        ...
        skeleton = '<span class="kategori-line"></span><ul class="list-kategori col-md-4">';
        for (var h = 0, cen = category.length; h < cen; ++h) {
            skeleton += '<li class="list-kategori-' + h + '"><a onclick="clickTabs_' + hash + '(' + h + ');return false;" onmousedown="return false;" href="javascript:;">' + category[h].term + '</a></li>';
        }
        skeleton += '</ul>';

        // Bulid the tabs contents skeleton
        skeleton += '<div class="judul-artikel col-md-8">';
